...
const HistoryReceiptCard = ({ item }) => {
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <View>
        {data?.Craccount == selectedAccount ? (
          <Text style={styles.amount}>{`₦${ thousandOperator(data?.Amount) }`  || "Not Available"}</Text>
        ) : (
          <Text style={[styles.amount, { color: "red" }]}>{`₦${ thousandOperator(data?.Amount) }`  || "Not Available"}</Text>
        )}
...
export default HistoryReceiptCard;
